...
| 规则 | 目标 | 位置 | 说明 |
|------|------|------|------|
| `DST-PORT,5443` | 全球直连 | **全部规则最前** | 5443 端口强制直连，优先级最高，任何域名/平台规则都不会将其抢走；也早于「非标端口」的 444-65535 区间规则，故不会被其接管 |

> 需要新增类似的端口强制规则，写成 `ruleset=目标组,[]DST-PORT,端口号`，
> 并放在模板「零、最高优先级」区块内即可。
...
- **⚠️ 内核启动失败：`can't download ASN.mmdb: context deadline exceeded`？**
  完整报错形如：
  ```
  level=fatal msg="Parse config error: rules[606] [IP-ASN,32934,Facebook,no-resolve]
        error: can't download ASN.mmdb: context deadline exceeded"
  ```
  **原因**：规则里含 `IP-ASN` 类型，它需要 **ASN.mmdb** 数据库。若 OpenClash 的
  `GEO 数据库订阅 → Geo ASN` 显示 `File Not Exist`，内核启动时会尝试联网下载；
  但此时**代理尚未启动**，下载必然超时 → 配置解析失败 → 内核起不来。
  这是典型的死锁：**要下载数据库得先有代理，要有代理得先加载配置**。

  **本模板的处理**：已彻底移除 `IP-ASN` 依赖，不为几条规则去拉一个额外数据库——
  | 清单 | 处理 | 覆盖如何保证 |
  |------|------|-------------|
  | Facebook | `Facebook_NoASN.list` | 569 条域名规则 + `geosite:facebook` |
  | Telegram | `Telegram_NoASN.list` | 域名规则 + `geosite:telegram` + **`GEOIP,telegram`**（走 GeoIP MMDB，无需额外库） |
  | AI 服务 | `AI_Service.list` | 域名规则 + `geosite:openai/anthropic/google-gemini` |

  **如果你自己加规则**：不要引入 `IP-ASN`，除非你确认 ASN.mmdb 已就位。

  **GEO 数据库该勾哪些自动更新**：
  - ✅ **GeoSite**、✅ **GeoIP MMDB** —— 分流准确性靠它们，建议开启自动更新；
  - ❌ **Geo ASN**、❌ **GeoIP Dat** —— 本模板完全不依赖，勾了只会周期性报下载失败。
