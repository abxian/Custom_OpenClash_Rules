...
| 文件 | 类型 | 作用 | 什么时候看/用 |
|------|------|------|--------------|
| [Custom_Ecommerce.ini](./Custom_Ecommerce.ini) | 订阅转换模板 | 24 个策略组，社媒/电商/支付/验证/物流全覆盖；平台清单 + GEOSITE 双保险；无广告拦截（便于投流） | subconverter 转换订阅时引用 |
...
> ⚠️ **验证服务要跟随账号 IP**：人机验证/风控/短信验证码若与账号出口不一致，是触发风控的常见原因。
> 多账号运营时不要依赖共享的 `验证服务` 组，应按设备用 `SRC-IP-CIDR` 让整机流量走同一 IP。
> 详见 [Custom_Ecommerce.md](./Custom_Ecommerce.md) 第 1.2 节。
